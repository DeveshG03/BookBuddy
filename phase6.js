document.addEventListener('DOMContentLoaded',()=>{
 ...
 // Replace photo placeholders automatically when the user later adds the exact files.
 ...
 // Photo lightbox.
 const box=document.createElement('div');box.className='photo-lightbox';box.innerHTML='<div class="lightbox-inner"><button class="lightbox-close" aria-label="Close">×</button><img class="lightbox-image" alt="Memory"><div class="lightbox-caption"></div></div>';document.body.appendChild(box);
 const lbImg=box.querySelector('.lightbox-image'),lbCap=box.querySelector('.lightbox-caption');
 document.querySelectorAll('.memory-card').forEach(card=>card.addEventListener('click',()=>{const img=card.querySelector('.photo-slot img');if(!img)return;lbImg.src=img.src;lbCap.textContent=card.querySelector('.caption b')?.textContent||'Memory';box.classList.add('open');document.body.style.overflow='hidden'}));
 ...
 // Music player: works automatically once assets/music/birthday-song.mp3 exists.
 ...
 // Tiny scrapbook heart trail on pointer movement, throttled.
