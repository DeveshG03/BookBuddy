...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 // CHANGE: Use encodeURI so the uploaded MP3 filename with spaces/parentheses resolves correctly on GitHub Pages.
 ...
 if(audio&&music&&play){
   const audioPath='assets/music/The Weeknd - Blinding Lights (Official Audio) - TheWeekndVEVO (youtube).mp3';
   audio.src=encodeURI(audioPath);
   audio.load();
   audio.addEventListener('loadedmetadata',()=>{music.dataset.ready='true';music.querySelector('.track-times span:last-child').textContent=format(audio.duration)});
   audio.addEventListener('error',()=>{music.dataset.ready='false';music.querySelector('.music-file').insertAdjacentHTML('beforeend',' <span class="audio-error">Audio could not be loaded. Please refresh the page.</span>')});
   audio.addEventListener('timeupdate',()=>{music.querySelector('.track-times span:first-child').textContent=format(audio.currentTime);const line=music.querySelector('.track-line');line.style.setProperty('--p',(audio.duration?(audio.currentTime/audio.duration)*100:0)+'%')});
   ...
   play.addEventListener('click',()=>{if(audio.paused)audio.play().catch(()=>{music.querySelector('.music-file').insertAdjacentHTML('beforeend',' <span class="audio-error">Tap the play button again after the page finishes loading.</span>')});else audio.pause()});
 }
